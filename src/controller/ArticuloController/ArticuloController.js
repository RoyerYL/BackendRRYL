...
const postArticulo = async (req, res) => {
  ...
  try {
    ...
    const [categoria, createdcategoria] = await Categoria.findOrCreate({
      where: { id: 0, nameCategoria: "No tiene categoria" }
    });

    // Find or create provider
    const [provedor, createdprovedor] = await Provedor.findOrCreate({
      where: { razonSocial: "No tiene provedor", nombreComercial: "No tiene provedor" }
    });
